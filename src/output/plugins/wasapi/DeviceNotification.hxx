...
/**
 * FIXME: assuming two wasapi outputs are configured with
 * explicit "device" fields set:
 * When playback is active on one output and a disconnected
 * device reconnects, the reconnected output will not resume
...
class WasapiDeviceNotification final : public IMMNotificationClient {
	 ...
	 * device_event.  This is needed for explicit device
	 * configurations, where any device state change may
	 * require reopening.
	 */
